...
export function CreateTaskDialog({ open, onOpenChange }) {
  const { toast } = useToast();
  const { user } = useAuth();
  const [date, setDate] = useState(null); // Initialize as null instead of undefined
  const [isLoading, setIsLoading] = useState(false);
  const [departments, setDepartments] = useState([]);
  const [users, setUsers] = useState([]);
  const [tasks, setTasks] = useState([]);
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant={"outline"}
                    className={cn(
                      "w-full justify-start text-left font-normal bg-white border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors",
                      !date && "text-muted-foreground"
                    )}
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                ...
                <PopoverContent
                  className="w-auto p-0 bg-white border border-gray-300 rounded-md shadow-lg z-[9999] -webkit-transform: translateZ(0); transform: translateZ(0);"
                  align="start"
                  side="bottom"
                  avoidCollisions
                >
